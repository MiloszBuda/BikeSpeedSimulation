...
export const demoSimulationResponse: WhatIfSimulationResponse = {
  summary: {
    ...
    simulated_time_s: 1284.0,
    time_delta_s: 48.0, // 48 seconds faster without wind
    baseline_avg_speed_kmh: 32.4,
    simulated_avg_speed_kmh: 33.6,
    equivalent_power_w: 228.5,
    pacing_mode: 'original',
    reverse_route: false,
    wind_scenario: 'Zero Wind (Calm)',
  },
  spatial_points: demoEnrichedPoints.map((p, idx) => ({
    distance_m: p.distance_m,
    lat: p.lat,
    lon: p.lon,
    elevation_m: p.elevation_m,
    slope: parseFloat(((Math.sin((idx / 360) * 4 * Math.PI) * 0.03)).toFixed(4)),
    bearing_deg: p.bearing_deg,
    power_w: p.power_w,
    wind_speed_mps: 0.0,
    wind_dir_deg: p.wind_direction_deg,
    simulated_speed_mps: p.speed_mps * 1.04,
    simulated_speed_kmh: p.speed_kmh * 1.04,
    baseline_speed_mps: p.speed_mps,
    baseline_speed_kmh: p.speed_kmh,
    delta_time_s: parseFloat(((idx / 360) * 48.0).toFixed(1)),
  })),
};
